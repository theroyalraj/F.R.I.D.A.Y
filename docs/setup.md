...
**Intended shape:** the **house machine** runs OpenClaw **backends** (Docker, Postgres, Redis, skill-gateway, pc-agent). Laptops reach it **only through a proxy** (`ngrok`, Tailscale, etc.): set `PC_AGENT_URL` / `FRIDAY_PC_AGENT_URL` to that **HTTPS base** (no trailing slash). On the **Mac (normal setup)**, the **Listen UI stays local**: run Vite dev (`npm run ui:dev` in `pc-agent`) so the UI is `http://127.0.0.1:5173` while **all API and SSE** are proxied to `PC_AGENT_URL` (see [`pc-agent/vite.config.ts`](../pc-agent/vite.config.ts)). **Speech, silence daemon (ECHO), ambient, mic listen, Cursor reply TTS, and macOS notification forwarding** run **on the Mac**; the server does not need to play your laptop audio.

- **Home (server only):** `npm run start:server-stack` (or `OPENCLAW_START_MODE=server` with `npm run start:all`). Keep `OPENCLAW_SKILL_GATEWAY_URL=http://127.0.0.1:3848` on the server. Tunnel pc-agent: `npm run tunnel:friday` → `ngrok http 3847`. Optionally tunnel the gateway: `npm run tunnel:ngrok` → `ngrok http 3848`.
- **Mac / laptop (recommended):** Same repo and `.env` as split-stack client: `PC_AGENT_URL=https://your-tunnel.example` (remote pc-agent). **Terminal 1:** `cd pc-agent && npm run ui:dev` — local Listen with proxy to the tunnel. **Terminal 2:** `npm run start:client-stack` (or keep using `OPENCLAW_START_MODE=client`). Set **`OPENCLAW_CLIENT_LISTEN_URL=http://127.0.0.1:5173`** so **`start.mjs`** opens the **local** dev UI instead of the remote `/friday/listen` page. Enable **`FRIDAY_MAC_NOTIFY_WATCH=true`** on Darwin so [`scripts/mac-notify-watch.py`](../scripts/mac-notify-watch.py) subscribes to `/voice/stream` and mirrors **`win_notify`** events into **Notification Center** (osascript). Use **`FRIDAY_EMAIL_WATCH=false`** on the laptop if Gmail watch runs on the server.
- **Remote (minimal):** Copy `.env` secrets (`PC_AGENT_SECRET`, etc.) from the server; set `PC_AGENT_URL` and `FRIDAY_PC_AGENT_URL` to the **public** pc-agent base. Set `OPENCLAW_START_MODE=client` or run `npm run start:client-stack`. Omit `OPENCLAW_CLIENT_LISTEN_URL` if you intentionally open Listen **on the tunnel host** (UI served by remote pc-agent).
...
- **Bootstrap:** [`scripts/openclaw-client-bootstrap.sh`](../scripts/openclaw-client-bootstrap.sh) — interactive menu includes **Open in Cursor only** (existing clone), optional **open folder in Cursor after** pull/setup/full, flags **`--open-cursor`** / **`--cursor-only --home PATH`**. Optional one-liner via public Gist raw (maintainer copy: `https://gist.githubusercontent.com/theroyalraj/d4ddf7b05d156271f9f3205e2cb101cb/raw/openclaw-client-bootstrap.sh`; pin a revision in production).
  - **Current clone — Cursor rules + Node + pip (no ngrok):** from the repo root, either run the same steps locally — `npm ci && python3 scripts/openclaw_company.py --generate-rule && python3 -m pip install -r scripts/requirements-cursor-reply-watch.txt` — or: `curl -fsSL 'https://gist.githubusercontent.com/theroyalraj/d4ddf7b05d156271f9f3205e2cb101cb/raw/openclaw-client-bootstrap.sh' | bash -s -- --setup-only --home "$(pwd)"` (add `--open-cursor` to launch Cursor on that folder after setup; `git pull` runs when `.git` exists).
  - **Copy rules only (wizard option 3):** copies `.cursor` from an existing clone into **the folder where you ran the script** (pwd). You must enter the **full path** to the clone (not `y` / `n`). Non-interactive: `--copy-cursor-only --home /path/to/openclaw`.
  - **Full client (wizard option 5):** optional **copy** of the install tree’s `.cursor` into the **directory where you ran the script** (default **yes** when that path differs from the install path). Cursor applies rules from the workspace folder you open. Non-interactive: `--copy-cursor-to-cwd` or `OPENCLAW_BOOTSTRAP_COPY_CURSOR_CWD=1`; override target with `OPENCLAW_BOOTSTRAP_INVOCATION_CWD`.
  - **Interactive:** `curl -fsSL 'https://gist.githubusercontent.com/theroyalraj/d4ddf7b05d156271f9f3205e2cb101cb/raw/openclaw-client-bootstrap.sh' | bash -s -- --interactive`. If bash reports **`invalid option nameipefail`**, the Gist was saved with Windows CRLF — use **`| tr -d '\r' |`** before **`bash`**, or republish the Gist from a LF copy (repo enforces LF for `*.sh` via `.gitattributes`). The wizard uses colorized steps, numbered choices, **live `GET /voice/ping`** after you paste the ngrok URL, and an optional **spoken line** (`friday-speak` or macOS `say`) once the tunnel passes. Menus read from `/dev/tty` (works when the script is piped). With **no arguments** from a real terminal, the same wizard runs. **Flags:** `--pull-only`, `--setup-only`, full client with `--ngrok-url`, `--no-setup`, `--no-voice-check`, `OPENCLAW_BOOTSTRAP_VOICE_CHECK=0` (honoured when not using the interactive wizard). See `bash …/openclaw-client-bootstrap.sh --help`.
...
## 10c) Managed / company laptop — no local install (browser only)

If IT policy blocks **git**, **npm**, **Homebrew**, or running install scripts on your Mac, you do **not** need the Gist bootstrap or the client stack on that machine. Treat the **home server** as the only place OpenClaw is installed and running.

- **On the server:** Same stack as **§10b Home** — pc-agent on **3847**, tunnel with `npm run tunnel:friday` (or reach the host via **Tailscale / VPN** on `http://<tailscale-ip>:3847` without public ngrok). **Skill-gateway** must be running if you want **GET `/openclaw/status`** to show green (Listen uses it); default `OPENCLAW_SKILL_GATEWAY_URL=http://127.0.0.1:3848`.
- **On the work Mac:** Open a browser to **`https://<your-pc-agent-base>/friday`** (voice UI) or **`/friday/listen`** (Listen app). No repo clone, no Cursor client stack, no `friday-listen.py` / `friday-speak.py` on the laptop.
- **Server checklist for Listen in the browser**

| Variable / condition | Why |
|--------|-----|
| Non-empty **`PC_AGENT_SECRET`** | Injects `window.__OPENCLAW_PC_AGENT_BEARER__` into `/friday` and `/friday/listen` so the SPA can call **`/voice/*`**, **`/settings/*`**, **`/integrations/*`**, etc. with the same Bearer daemons use. Without it, you need a normal login JWT only. |
| **`JWT_SECRET`** + **Postgres** (`OPENCLAW_DATABASE_URL`) + at least one org user | Required for **sign-in** and for **`POST /auth/auto-session`** when **`PC_AGENT_LISTEN_AUTO_LOGIN=true`**. With only agent injection and no DB, Listen still uses the injected secret for API calls (no user profile). |
| **`PC_AGENT_LISTEN_AUTO_LOGIN=true`** (optional) | On first load, mints a user JWT using the injected agent Bearer (still requires DB + membership). |
| **`PC_AGENT_PUBLIC_BASE_URL`** (optional) | If set (HTTPS tunnel base, no trailing slash), **`GET /health`** and **`GET /openclaw/status`** embed that host in **`links`** instead of guessing from **`X-Forwarded-*`**. Behind ngrok, forwarded headers usually suffice without this. |
| Persona edits | **`PUT` / `DELETE /settings/personas`** accept **agent Bearer** or **org admin JWT** only — ordinary members can read personas but not patch the global registry. |

- **Auth:** When `PC_AGENT_SECRET` is set, the server injects it into those HTML pages as an in-page Bearer for API calls. Anyone who can open the same URL effectively has that capability — treat the tunnel URL like a password. Prefer **Tailscale** or **ngrok OAuth / IP restrictions** if the laptop must stay off the public internet for admin tools.
- **What you do not get without a client install:** Local **Cursor** reply TTS (`cursor-reply-watch`), Python **listen** daemons bound to the laptop mic, and **npm run start:client-stack** ambient — those stay on machines where you can run the repo. **Python `friday-speak` on the server** plays on the **server’s** audio device; UI paths that stream TTS to the browser still use the browser’s speakers. Mic capture in **Listen** works only if the browser session is allowed to use the mic (and your org permits it).
