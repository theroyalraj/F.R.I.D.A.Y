...
- **Bootstrap:** [`scripts/openclaw-client-bootstrap.sh`](../scripts/openclaw-client-bootstrap.sh) — host the raw file on a GitHub Gist for one-liner install. **Modes:** `--pull-only` (git clone or pull only), `--setup-only` (refresh `.cursor` company rule via `openclaw_company.py`, `npm ci`, client `pip` deps), or default full client (also `--ngrok-url`, upsert `.env`, celebration, `start:client-stack`). **`--no-setup`** on the full run skips `npm ci` / pip if you already ran `--setup-only`. Set `OPENCLAW_REPO_URL` or pass `--repo`. See `bash …/openclaw-client-bootstrap.sh --help`.
