...
def stream_unified_chat(
    ...
    *,
    ...
    def _one_stream(*, use_http2: bool) -> tuple[str, bool]:
        """Returns (cleaned_text, transport_ok). transport_ok False → try alternate HTTP version."""
        full: List[str] = []
        chunk_i = 0
        first_text_chunk = True
        conn_tag = "t2_conn" if use_http2 else "t2_conn_http11"
        try:
            with httpx.Client(http2=use_http2, timeout=httpx.Timeout(120.0, connect=30.0)) as client:
            ...
            slog.emit("t_error")
            print(f"[gRPC] stream error (http2={use_http2}): {e}", flush=True)
            return "", False
    ...
    raw = os.environ.get("FRIDAY_CURSOR_GRPC_RETRY_HTTP11", "true").strip().lower()
    allow_http11 = raw not in ("0", "false", "no", "off")

    cleaned, ok = _one_stream(use_http2=True)
    if not ok and allow_http11:
        slog.emit("t_retry_http11")
        cleaned, ok = _one_stream(use_http2=False)
    if not ok:
        return ""
    return cleaned
